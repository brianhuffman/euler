...
repeated_digit_primes :: Integer -> Int -> Int -> [Integer]
repeated_digit_primes d n m =
  [ p |
    qs <- subseqs_len m ps,
    let mask = ones - sum (map (10^) qs),
    rs <- sequence [ [0 .. 9] | q <- qs ],
    let p = d*mask + sum (zipWith (\q r -> r*10^q) qs rs),
    p >= 10^(n-1),
    is_prime p
  ]
  where
...
max_repeated_digit_primes :: Integer -> Int -> [Integer]
max_repeated_digit_primes d n = f 0
  where
    f m = let ps = repeated_digit_primes d n m
          in if null ps then f (m+1) else ps

all_repeated_digit_primes :: Int -> [Integer]
all_repeated_digit_primes n =
  concatMap (\d -> max_repeated_digit_primes d n) [0 .. 9]

prob111 :: Int -> Integer
prob111 n = sum (all_repeated_digit_primes n)

main :: IO String
main = return $ show $ prob111 10
-- 612407567715
