...
mkseg :: Cuboid -> Seg
mkseg (x0,y0,z0,x1,y1,z1) = (x0,x1)

seg_length :: Seg -> Integer
seg_length (x0,x1) = toInteger (x1-x0)

mkrect :: Cuboid -> Rect
mkrect (x0,y0,z0,x1,y1,z1) = (x0,y0,x1,y1)

rect_area :: Rect -> Integer
rect_area (x0,y0,x1,y1) = seg_length (x0,x1) * seg_length (y0,y1)

type Seg = (Int, Int)
type Rect = (Int, Int, Int, Int)
type Cuboid = (Int, Int, Int, Int, Int, Int)

total_length :: [Seg] -> Integer
total_length segs = event_length (sort (concatMap events segs))
  where events (x0,x1) = [(x0,1), (x1,-1)]
    
event_length :: [(Int, Int)] -> Integer
event_length es = f 0 0 0 es
  where
    f t k x [] = t
    f t k x ((x',d):es) = f t' (k+d) x' es
      where t' = if k > 0 then t + toInteger (x'-x) else t

total_area :: [Rect] -> Integer
total_area rects = event_area (sort (concatMap events rects))
  where events (x0,y0,x1,y1) = [((x0,(y0,y1)),1), ((x1,(y0,y1)),-1)]

event_area :: [((Int, Seg), Int)] -> Integer
event_area es = go 0 Map.empty 0 es
  where
    inc, dec :: Int -> Maybe Int -> Maybe Int
    inc n Nothing = Just n
    inc n (Just m) = if n+m == 0 then Nothing else Just (n+m)
    dec n = inc (-n)

    go :: Integer -> Map Int Int -> Int -> [((Int, Seg), Int)] -> Integer 
    go t k x [] = t
    go t k x (((x',(y0,y1)),d):es) = go t' k' x' es
      where
        k' = Map.alter (dec d) y1 . Map.alter (inc d) y0 $ k
        t' = t + toInteger (x'-x) * event_length (Map.toAscList k)
