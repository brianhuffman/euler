module PascalPyramid where
import EulerLib (funArray, pascal_triangle)
...
lengthRLE :: RLE a -> Int
lengthRLE = sum . map snd

scaleRLE :: Int -> RLE a -> RLE a
...
bindRLE :: RLE a -> (a -> RLE b) -> RLE b
bindRLE xs f = concat $ listRLE [ (f x, n) | (x, n) <- xs ]
...
2012 "------222111111222111111222------222111111222111111222------"
 ...
 120 " -   1   1   -   1   1   -   2   2   -   1   1   -   1   1   -"
 121 "-   -   1   -   -   1   -   -   2   -   -   1   -   -   1   -   -"

1210 "-11 -22 122 -11 -22 122 -11 -33 233 -11 -22 122 -11 -22 122 -11 -"
1211 "--1 --2 112 --1 --2 112 --1 --3 223 --1 --2 112 --1 --2 112 --1 --"
1212 "--- --- 111 --- --- 111 --- --- 222 --- --- 111 --- --- 111 --- ---"
...
1100 "-22 122 122 -33 233 233 133 233 233 -22 122 122 -"
1101 "--2 112 112 --3 223 223 113 223 223 --2 112 112 --"
1102 "--- 111 111 --- 222 222 111 222 222 --- 111 111 ---"

-}

pascal_prime_power :: (Integral a) => a -> a -> [Int]
pascal_prime_power p n
  | False = undefined
  where
    (q, r) = divMod n p
    qs = pascal_prime_power p q
    rs = pascal_prime_power p r
